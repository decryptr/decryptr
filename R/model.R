...
#' a local or remote file create by the user with [train_model()].
#'
...
load_model <- function(model, labs = c(0:9, base::letters)) {
  ...
  if (!(model %in% c("rfb", "trt", "tjmg", "esaj", "rsc", "cadesp", "nfesp", "tjes", "tjrs", "jucesp"))) {
    ...
    url <- sprintf("https://storage.googleapis.com/decryptr/models/%s.hdf5", model)
    file_path <- pins::pin(url)
...
## WILL IMPLEMENT GENERATORS AGAIN LATER
